...
import { kitabIcon } from '../components/CustomIcons';
import { kitabBooks, KitabBook } from '../utils/kitabBooks';
import { Search, BookOpen, User } from 'lucide-react';

const KitabPage = () => {
  const { t, setHeaderTitle, settings } = useAppStore();
  const [searchQuery, setSearchQuery] = useState('');
  const [books, setBooks] = useState<KitabBook[]>(kitabBooks);

  useEffect(() => {
    setHeaderTitle(t('kitab'));
  }, [t, setHeaderTitle]);
  ...
  return (
    <div className="pb-20 space-y-6">
      ...
      <div className="bg-blue-50 dark:bg-blue-900/10 p-6 rounded-2xl flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('kitab')}</h1>
          <p className="text-gray-500 dark:text-gray-400 text-sm">{t('kitabDesc')}</p>
        </div>
      </div>
